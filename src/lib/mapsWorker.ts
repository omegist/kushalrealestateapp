...
export async function geocodeAndStore(propertyId: string, address: string): Promise<{ lat: number; lng: number }> {
...
export async function getPropertyCoords(propertyId: string): Promise<{ lat: number; lng: number } | null> {
...
export async function getAllPropertyCoords(): Promise<{ property_id: string; lat: number; lng: number }[]> {
...
export function googleMapsDirectionsUrl(destLat: number, destLng: number): string {
